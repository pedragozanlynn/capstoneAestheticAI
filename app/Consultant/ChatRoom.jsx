// ✅ UPDATED ONLY (header + footer safe-area / navbar overlap fix + keyboard lift):
// - Header respects top safe area (Android status bar / iOS notch)
// - Footer respects bottom safe area (Android nav bar / iOS home indicator)
// - ✅ Footer also lifts above keyboard using keyboardHeight
...
export default function ChatRoom() {
  ...
  // ✅ NEW: keyboard height to lift footer above keyboard
  ...
  // ✅ computed top padding for header (prevents notch/status overlap)
  const headerTopPad =
    Platform.OS === "android"
      ? Math.max(insets.top, StatusBar.currentHeight || 0)
      : insets.top;

  // ✅ base bottom padding (original intent)
  const baseBottomPad = Platform.OS === "ios" ? 35 : 15;
  ...
  return (
    <View style={styles.mainContainer}>
      ...
      <View style={{ flex: 1 }}>
        ...
        {/* ✅ FIX: footer now has safe bottom + lifts above keyboard */}
        <View
          style={[
            styles.footer,
            {
              paddingBottom: Math.max(insets.bottom, baseBottomPad) + (keyboardHeight || 0),
            },
          ]}
        >
...
const styles = StyleSheet.create({
  ...
  header: {
    ...
    // ✅ we apply top padding inline using insets; keep only paddingBottom here
    paddingBottom: 10,
    borderBottomWidth: 1,
  ...
  footer: {
    ...
    paddingTop: 10,
    // ✅ baseline only; final paddingBottom is applied inline using insets + keyboardHeight
